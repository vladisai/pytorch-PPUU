...
class I80Car(Car):
    ...
    def __init__(
        ...
        self._max_t = (
            len(x) - np.count_nonzero(np.isnan(x)) - 2
        )  # 2 for computing the acceleration
    ...
    def _get(self, what, k):
        ...
        if (
            what == "init_direction"
        ):  # valid direction can be computed when speed is non-zero
            t = 1  # check if the car is in motion the next step
            while (
                self._df.at[self._df.index[t], "Vehicle Velocity"] < 5
                and t < self._max_t
            ):
                t += 1
...
class I80(Simulator):
    ...
    def __init__(self, **kwargs):
        ...
        assert (
            delta_t >= 1 / 10
        ), f"Minimum delta t is 0.1s > {delta_t:.2f}s you tried to set"
        assert (
            delta_t * 10
        ).is_integer(), f"dt: {delta_t:.2f}s must be a multiple of 0.1s"
    ...
    def _get_data_frame(self, time_slot, x_max, x_offset):
        ...
        file_name = f"{self.dataset_path}/../../xy-trajectories/{time_slot}"  # noqa: E501
    ...
    def reset(
        ...
        if train_only:
            ...
            assert not (
                frame or time_slot or vehicle_id
            ), "Already selecting training episode from file."
            time_slot, vehicle_id = self.train_indx[
    ...
    def step(self, policy_action=None):
        ...
        if self.normalise_action and policy_action is not None:
            ...
            np.add(
                policy_action, self.data_stats["a_mean"], policy_action
            )  # add the mean

        df = self.df
        now = df["Frame ID"] == self.frame
        vehicles = (
            set(df[now]["Vehicle ID"])
        ...
        if vehicles:
            ...
            for vehicle_id in vehicles:
                ...
                if (
                    self.controlled_car
                    and not self.controlled_car["locked"]
                    and self.frame >= self.controlled_car["frame"]
                    and (
                        self.controlled_car["v_id"] is None
                        or vehicle_id == self.controlled_car["v_id"]
                    )
                ):
        ...
        for v in self.vehicles:
            ...
            right_vehicles = (
                self._get_neighbours(lane_idx, +1, v)
                if lane_idx < 5 or lane_idx == 5 and v.front[0] > 18 * LANE_W
                else None
            )
    ...
    def _draw_lanes(self, surface, mode="human", offset=0):
        ...
        if mode == "human":
            ...
            draw_line(
                ...
                w,
                (0, bottom + 53),
                (60 * LANE_W, bottom + 53 - slope * 60 * LANE_W),
                3,
            )
        ...
        if mode == "machine":
            ...
            draw_line(
                ...
                w,
                (m, bottom + 29),
                (m + 18 * LANE_W, bottom + 29 - slope * 18 * LANE_W),
                1,
            )
            draw_line(
                s,
            ...
            # offroad regions
            pygame.Surface.fill(
                s, b, pygame.Rect(m + 0, m + lanes[0]["min"] - 35, sw, 34)
            )
            pygame.draw.polygon(
                s,
                b,

...
class DimensionalityReduction:
    ...
    def __init__(self):
        """Lazy initialization, in order to not waste time every time we open
        the notebook"""
    ...
    def initialize(self):
        ...
        if self.initialized:
            return
        if os.path.exists(
            DimensionalityReduction.SAVE_DIMENSIONALITY_REDUCTION_PATH
        ):
            with open(
                DimensionalityReduction.SAVE_DIMENSIONALITY_REDUCTION_PATH,
                "rb",
            ...
            features = (
                DimensionalityReduction.get_dimensionality_reduction_data()
            )
    ...
    def get_model_failing_features(experiment, seed, checkpoint):
        ...
        for fail in failing:
            ...
            features = np.append(
                features, [costs[fail - 1]["collisions_per_frame"][length - 1]]
            )
            features = np.append(
    ...
    def get_dimensionality_reduction_data():
        ...
        for seed in seeds:
            ...
            for checkpoint in checkpoints:
                try:
                    if data == []:
                        data = DimensionalityReduction.get_model_failing_features(  # noqa: E501
                        ...
                        data = np.concatenate(
                            [
                                data,
                                DimensionalityReduction.get_model_failing_features(  # noqa: E501
                                    experiment, seed, checkpoint
                                ),
                            ]
                        )

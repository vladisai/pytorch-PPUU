...
class DimensionalityReduction:
    ...
    def initialize(self):
        ...
        if os.path.exists(DimensionalityReduction.SAVE_DIMENSIONALITY_REDUCTION_PATH):
            with open(
                DimensionalityReduction.SAVE_DIMENSIONALITY_REDUCTION_PATH, "rb"
            ) as f:
            ...
            self.dimensionality_reduction = sklearn.manifold.TSNE(n_components=2)
            self.dimensionality_reduction.fit(features)
            self.km = sklearn.cluster.KMeans(n_clusters=5).fit(features)
            with open(
                DimensionalityReduction.SAVE_DIMENSIONALITY_REDUCTION_PATH, "wb"
            ) as f:
    ...
    def get_episode_features(experiment, seed, checkpoint, episode):
        ...
        features = []
        features.append(
            DataReader.get_episode_speeds(experiment, seed, checkpoint, episode)[
                -history_size:
            ]
        )
    ...
    def get_model_failing_features(experiment, seed, checkpoint):
        ...
        for fail in failing:
            ...
            features = np.append(
                features,
                DataReader.get_last_gradient(experiment, seed, checkpoint, fail),
            )
